...
## Claim: Hysteresis and Motion Gating prevent boundary oscillation and transient false positives.
- **Source**: Control Systems Theory and HCI (Human-Computer Interaction)
- **Support**: In continuous signal classification, users hovering near a threshold will cause the output state to rapidly toggle back and forth ("boundary oscillation"). Implementing hysteresis (using a higher threshold to enter a state and a lower threshold to leave it) stabilizes the output. Additionally, human posture is not perfectly static; brief, high-velocity movements (reaching for a mouse, shifting weight) are transient and should not be penalized as structural posture degradation.
- **Implementation Consequence**: We added a `motionStability` gate in `TemporalFilter` that scales down posture penalties inversely to feature velocity. We also implemented strict hysteresis thresholds (Penalty > 120 to drift, Penalty < 70 to recover) to guarantee stable, predictable state transitions.