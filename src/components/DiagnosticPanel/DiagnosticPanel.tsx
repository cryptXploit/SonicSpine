...
export function DiagnosticPanel({
  ...
  return (
    <div className="absolute top-4 left-4 bg-black/80 text-green-400 p-4 rounded text-xs font-mono w-64 z-50 overflow-hidden" data-testid="diagnostic-panel">
      ...
      {deviations && (
        <div className="mb-2">
          <h4 className="text-green-600 font-bold">Deviations</h4>
          ...
          <div className="flex justify-between"><span>Crane:</span> <span>{deviations.craneDeviation.toFixed(3)}</span></div>
          <div className="flex justify-between"><span>Collapse:</span> <span>{deviations.collapseDeviation.toFixed(3)}</span></div>
          <div className="flex justify-between mt-1 pt-1 border-t border-green-800">
            <span className="font-bold">Penalty:</span> 
            <span className={`font-bold ${deviations.totalPenalty > 100 ? 'text-red-400' : ''}`}>
